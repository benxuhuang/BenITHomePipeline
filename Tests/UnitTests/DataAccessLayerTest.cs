...
namespace Tests.UnitTests
{
    public class DataAccessLayerTest
    {
        ...
        {
            using (var db = new BenITHomeContext(Utilities.TestDbContextOptions()))
            {
                // Arrange
                var expectedMessages = BenITHomeContext.GetSeedingArticles();
        ...
        public async Task DeleteAllMessagesAsync_MessagesAreDeleted()
        {
            ...
            {
                // Arrange
                var seedMessages = BenITHomeContext.GetSeedingArticles();
                ...
                // Assert
                Assert.Empty(await db.Article.AsNoTracking().ToListAsync());
            }
        }
        ...
        public async Task DeleteMessageAsync_MessageIsDeleted_WhenMessageIsFound()
        {
            using (var db = new BenITHomeContext(Utilities.TestDbContextOptions()))
            {
                ...
                #region snippet3
                // Assert
                var actualMessages = await db.Article.AsNoTracking().ToListAsync();
                Assert.Equal(
                    expectedMessages.OrderBy(m => m.ID).Select(m => m.Title),
                    actualMessages.OrderBy(m => m.ID).Select(m => m.Title));
                #endregion
